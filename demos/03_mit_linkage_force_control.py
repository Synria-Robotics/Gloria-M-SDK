...
"""
MIT linkage gripper force control demo.

This demo targets a "linkage-driven gripper" scenario.  Because fingertip
force and motor torque are not in a simple 1-to-1 relationship, a piecewise-
linear "equivalent moment-arm vs. motor angle" curve is used to approximate
the transmission ratio:

    fingertip_force ~= closing_torque / equivalent_moment_arm

Control flow:
1. Apply positive torque to open the gripper toward its upper limit.
2. Apply negative torque to approach the object in the closing direction.
3. Detect contact via estimated grip force, velocity stall, or close-limit.
4. Enter the hold phase: use MIT torque control to maintain target grip force.
5. After the hold duration expires, apply positive torque to release.

The default moment-arm profile is a placeholder; replace it with calibration
data from your own linkage mechanism before use.
"""
