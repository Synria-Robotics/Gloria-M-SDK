"""
Integration-style tests for MotorClient using FakeCanAdapter.
...
class TestConnection:
    ...
    def test_connect_apply_limits_sends_frames(self, fake: FakeCanAdapter) -> None:
        ...
        with MotorClient("unused", _transport=fake, limits=limits) as g:
...
class TestMotorCommands:
    ...
    def test_set_mode_succeeds_when_motor_echoes_back(
        ...
        gripper.set_mode(ControlMode.POS_VEL)
...
class TestMotionCommands:
    def _setup_pv_mode(self, gripper: MotorClient, fake: FakeCanAdapter) -> None:
        fake.queue_param_reply(can_id=0x101, rid=int(Variable.CTRL_MODE),
                               value=int(ControlMode.POS_VEL), is_u32=True)
        gripper.set_mode(ControlMode.POS_VEL)

    def _setup_mit_mode(self, gripper: MotorClient, fake: FakeCanAdapter) -> None:
        fake.queue_param_reply(can_id=0x101, rid=int(Variable.CTRL_MODE),
    ...
    def test_send_pos_vel_requires_pv_mode(
        ...
        with pytest.raises(GloriaModeError, match="POS_VEL"):
            gripper.send_pos_vel(position=1.0, velocity=1.0)
    ...
    def test_send_pos_vel_sends_frame(
        ...
        fake.clear()
        gripper.send_pos_vel(position=1.5, velocity=1.0, poll=False)
    ...
    def test_send_mit_requires_mit_mode(
        ...
        with pytest.raises(GloriaModeError, match="MIT"):
            gripper.send_mit(kp=0.0, kd=0.5, q=0.0, dq=0.0, tau=-1.0)
    ...
    def test_send_mit_sends_frame(
        ...
        self._setup_mit_mode(gripper, fake)
        fake.clear()
        gripper.send_mit(kp=0.0, kd=0.5, q=0.0, dq=0.0, tau=-1.0, poll=False)
    ...
    def test_position_clamped_to_safe_range(self, fake: FakeCanAdapter) -> None:
        ...
        with MotorClient("unused", _transport=fake, safe_position=safe) as g:
            fake.queue_param_reply(can_id=0x101, rid=int(Variable.CTRL_MODE),
                                   value=int(ControlMode.POS_VEL), is_u32=True)
            g.set_mode(ControlMode.POS_VEL)
            fake.clear()
            g.send_pos_vel(position=99.0, velocity=1.0, poll=False)
...
class TestParamCommands:
    def test_read_returns_value_when_motor_replies(
        ...
        result = gripper.read_param(rid, timeout_s=0.1)
    ...
    def test_read_returns_none_on_timeout(
        ...
        result = gripper.read_param(int(Variable.PMAX), timeout_s=0.01)
    ...
    def test_write_f32_sends_frame(
        ...
        fake.clear()
        gripper.write_param_f32(int(Variable.PMAX), 3.14)
        assert len(fake.sent_frames) == 1
